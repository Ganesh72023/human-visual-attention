import type { EmotionLabel } from "../models/Upload";

export function deriveCognitiveState(emotion: EmotionLabel, behaviors: string[]): string {
  if (emotion === "nervous") return "Heightened arousal / anxiety";
  if (emotion === "angry") return "Frustration / threat response";
  if (emotion === "sad") return "Low mood / withdrawal";
  if (emotion === "happy") return "Positive affect / engagement";
  if (behaviors.includes("fidgeting")) return "Restless attention";
  return "Neutral / baseline";
}

export function generateSuggestions(emotion: EmotionLabel, behaviors: string[]): string[] {
